...
**GPT-6 Luna / high 复测——2026-09-26。** [新增 10 次离线实测](evals/results/2026-09-26/luna6-content/REPORT.md)检查文件正文缺失、实际内容读取、本地测试复用和版本边界。对照的两个 Skill 版本都能区分响应标题与实际证据；澄清假设练习的范围后，两版都能复用材料完整的快照。初始正向样例存在范围歧义，结果予以保留。V8 已补跑此前未执行的四个回归场景，但没有重跑真实架构搜索。[此前 9 次实测](evals/results/2026-09-26/luna6-high/REPORT.md)的失败记录仍保留；这些结果不代表始终遵守规则或全部通过。
