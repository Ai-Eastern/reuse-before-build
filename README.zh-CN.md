...
**GPT-6 Luna / high 复测——2026-09-26。** [三个 Skill 修订共 9 次实测](evals/results/2026-09-26/luna6-high/REPORT.md)检查版本对应关系、有效测试证据和最终决策的一致性。最终数据分析场景能将核查不完整的候选保持为 Blocked；队列场景不再用特殊数据库模式的测试证明普通 PostgreSQL 行为。仍发现未取得文件正文却写成已核查的表述。宿主达到新 Agent 数量上限，另外四个最终版回归场景仅完成准备、未启动。当前行为覆盖仍不完整，未宣称全部通过。
