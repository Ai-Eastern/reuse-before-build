...
**GPT-6 Luna / high follow-up — 2026-09-26.** [9 runs across three Skill revisions](evals/results/2026-09-26/luna6-high/REPORT.md) tested version linkage, meaningful test evidence and final decision consistency. The final analytics run kept an incompletely verified candidate Blocked; the queue run no longer used a special database-mode test as ordinary PostgreSQL evidence. Some evidence descriptions still overstated the returned file contents. Four final-version regressions were prepared but not started because the host reached its new-agent limit. This is incomplete behavior coverage, not an all-pass result.
