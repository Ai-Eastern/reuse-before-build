...
**Native Codex acceptance — 2026-09-26, GPT-6 Luna / high.** The [native-host report](evals/results/2026-09-26/native-host/REPORT.md) separates automatic Skill selection from correct behavior, exercises actual manual compaction and fresh-session handoff, and checks an isolated slice of real repository code. **The current revision did not pass every check:** it still adopted a candidate with required evidence unread, and the real-source task did not load this Skill. Failed trials and revision changes remain visible. This is native app-server evidence; other hosts, automatic context-pressure compaction and production integration remain untested.
