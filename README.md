...
**GPT-6 Luna / high — 2026-09-26.** The [latest evaluation](evals/results/2026-09-26/luna6-independent/REPORT.md) passed the scoped behavior criteria in **16 separate fresh contexts** on the same Skill revision: two live designs, twelve local scenarios and two evidence-boundary probes. Agents reused sufficient tests, added a missing test that caught a deliberate defect, checked historical results and respected handoff permissions. Analytics kept adoption blocked at an unverified resource limit; queue design retained deployment conditions. The report records a minor wording issue and command inefficiencies. These results do not establish automatic activation, production integration or a success rate. [Earlier failures and continuation-only coverage](evals/results/2026-09-26/luna6-closure/REPORT.md) remain available.
